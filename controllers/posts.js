...
module.exports = {
  ...
  getListings: async (req, res) => {
    try {
      const posts = await Post.find({ user: req.user.id });
      res.render("my-listings.ejs", { posts: posts, user: req.user });
    } catch (err) {
      console.log(err);
    }
  },
  getBookmarks: async (req, res) => {
    try {
      ...
      res.render("favorite-items.ejs", { posts: posts, user: req.user });
    } catch (err) {
      console.log(err);
    }
  },
  ...
  getCart: async (req, res) => {
    try {
      const posts = await Post.find({ carts: req.user.id });
      res.render("cart.ejs", { posts: posts, user: req.user });
    } catch (err) {
      console.log(err);
    }
  },
  addCart: async (req, res)=>{
    ...
    try{
      var post = await Post.findById({_id:req.params.id})
      cart = (post.carts.includes(req.user.id))
    } catch(err){
    }
    //if already bookmarked we will remove user from likes array
    if(cart){
      try{
        await Post.findOneAndUpdate({_id:req.params.id},
          {
            $pull : {'carts' : req.user.id}
          })
          
          console.log('Removed user from carts array')
          res.redirect('back')
        }catch(err){
          console.log(err)
        }
      }
      //else add user to carts array
      else{
        try{
          await Post.findOneAndUpdate({_id:req.params.id},
            {
              $addToSet : {'carts' : req.user.id}
            })
            
            console.log('Added user to carts array')
            res.redirect(`back`)
        }catch(err){
            console.log(err)
        }
